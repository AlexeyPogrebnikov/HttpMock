...
namespace HttpMock.Server.Tests
{
	...
	public class ConsoleArgsTests
	{
		...
		{
			var exception =
				Assert.Throws<InvalidConsoleArgsException>(() => new ConsoleArgs(new[] {"1.json", "2.json"}));

			Assert.AreEqual(
				"The path to the file with server settings must be specified only once. Path1: '1.json', Path2: '2.json'.",
				exception.Message);
		}
		...
		{
			string[] args = {"-host", "192.168.10.138", "-host", "127.0.0.1"};
			var exception = Assert.Throws<InvalidConsoleArgsException>(() => new ConsoleArgs(args));

			Assert.AreEqual(
				"The host parameter must be specified only once. Host1: '192.168.10.138', Host2: '127.0.0.1'.",
				exception.Message);
		}
		...
		{
			string[] args = {"-port", "80", "-port", "443"};
			var exception = Assert.Throws<InvalidConsoleArgsException>(() => new ConsoleArgs(args));

			Assert.AreEqual("The port parameter must be specified only once. Port1: '80', Port2: '443'.",
				exception.Message);
		}
		...
		{
			var exception = Assert.Throws<InvalidConsoleArgsException>(() => new ConsoleArgs(new[] {"-host"}));

			Assert.AreEqual("After the -host keyword an IP address must be specified (e.g., -host 127.0.0.1).",
				exception.Message);
		}
		...
		{
			var exception = Assert.Throws<InvalidConsoleArgsException>(() => new ConsoleArgs(new[] {"-port"}));
